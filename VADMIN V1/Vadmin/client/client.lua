...
RMenu.Add('MenuAdmininspecteur', 'main', RageUI.CreateMenu("ULTIMATEV ADMIN", " "))
RMenu:Get('MenuAdmininspecteur', 'main'):SetRectangleBanner(33, 33, 153)
RMenu.Add('MenuAdmininspecteur', 'staff', RageUI.CreateMenu("Réservé Staff", " "))
RMenu:Get('MenuAdmininspecteur', 'staff'):SetRectangleBanner(33,33, 153)
RMenu:Get('MenuAdmininspecteur', 'main'):SetSubtitle("Bienvenue ~g~".. GetPlayerName(PlayerId()) .. '')
...
function no_clip()
    ...
    if noclip then
      ...
      ESX.ShowAdvancedNotification("¦ULTIMATEV ", "", "NoClip : ~g~activé", "CHAR_STRETCH", 1)
    else
      SetEntityInvincible(ped, false)
      SetEntityVisible(ped, true, false)
      ESX.ShowAdvancedNotification("¦ULTIMATEV ", "", "NoClip : ~r~désactivé", "CHAR_STRETCH", 1)
    end
  end
...
function admin_tp_marker()
  ...
  ESX.ShowAdvancedNotification("¦ULTIMATEV ", "", "TP sur Marqueur : ~g~Réussi !", "CHAR_STRETCH", 1)
    else
  ESX.ShowAdvancedNotification("¦ULTIMATEV ", "", "~r~Aucun Marqueur !", "CHAR_STRETCH", 1)
    end
end
  ...
  function GiveCash()
    ...
    if amount ~= nil then
        ...
        if type(amount) == 'number' then
            TriggerServerEvent('¦ULTIMATEV :GiveCash', amount)
        end
    end
end
...
function GiveBanque()
    ...
    if amount ~= nil then
        ...
        if type(amount) == 'number' then
            TriggerServerEvent('¦ULTIMATEV :GiveBanque', amount)
        end
    end
end
...
function GiveND()
    ...
    if amount ~= nil then
        ...
        if type(amount) == 'number' then
            TriggerServerEvent('¦ULTIMATEV :GiveND', amount)
        end
    end
end
...
function admin_mode_fantome()
    ...
    if invisible then
          SetEntityVisible(ped, false, false)
          ESX.ShowAdvancedNotification("¦ULTIMATEV ", "", "Invisibilité : ~g~activé", "CHAR_STRETCH", 1)
      else
          SetEntityVisible(ped, true, false)
          ESX.ShowAdvancedNotification("¦ULTIMATEV ", "", "Invisibilité : ~r~désactivé", "CHAR_STRETCH", 1)
    end 
  end
  ...
  function admin_vehicle_flip()
    ...
    ESX.ShowAdvancedNotification("¦ULTIMATEV ", "", "~g~Véhicule retourné", "CHAR_STRETCH", 1)
...
function admin_godmode()
    ...
    if godmode then -- activé
          SetEntityInvincible(ped, true)
    ESX.ShowAdvancedNotification("¦ULTIMATEV ", "", "Invincibilité : ~g~activé", "CHAR_STRETCH", 1)
      else
          SetEntityInvincible(ped, false)
    ESX.ShowAdvancedNotification("¦ULTIMATEV ", "", "Invincibilité : ~r~désactivé", "CHAR_STRETCH", 1)
    end
  end
...
Citizen.CreateThread(function()
    while true do
                   ...
                   RageUI.IsVisible(RMenu:Get('MenuAdmininspecteur', 'main'), true, true, true, function()
                    RageUI.Separator("")
                    RageUI.Checkbox("∑ ~o~Activation ~h~STAFF",nil, service,{},function(Hovered,Ative,Selected,Checked)
                    RageUI.Separator("")
                        if (Selected) then
...
RageUI.Separator(""..#players.."/128 joueur(s) en ligne ")
                        RageUI.Button("~c~→~s~ Administratif", nil, {RightLabel = "→→→"},true, function()
                        end, RMenu:Get('MenuAdmininspecteur', 'main7'))

                        RageUI.Button("~y~→~s~ Personnel", nil, {RightLabel = "→→→"},true, function()
                        end, RMenu:Get('MenuAdmininspecteur', 'main3'))

                        RageUI.Button("~p~→~s~ Véhicule", nil, {RightLabel = "→→→"},true, function()
                        end, RMenu:Get('MenuAdmininspecteur', 'main4'))

                        RageUI.Button("¦ →~r~              ~h~Important", nil, {RightLabel ="→→→"},true, function()
...
RageUI.IsVisible(RMenu:Get('menu', 'options'), true, true, true, function()
    ...
    RageUI.Separator("~s~- ~r~Adminstration ~s~-")
    if superadmin then
        RageUI.Button("→~s~ Setjob le Joueur ", nil, {RightLabel = nil}, true, function(Hovered, Active, Selected)
            if (Selected) then
    ...
    RageUI.Button("→~s~Se téléporter au Joueur", nil, {}, true, function(Hovered, Active, Selected)
        if (Selected) then
            SetEntityCoords(PlayerPedId(), GetEntityCoords(GetPlayerPed(GetPlayerFromServerId(IdSelected))))
            ESX.ShowAdvancedNotification("¦ULTIMATEV ", "", '~b~Vous venez de vous téléporter à~s~ '.. GetPlayerName(GetPlayerFromServerId(IdSelected)) ..'', "CHAR_STRETCH", 1)

        end
    end)
    RageUI.Button("→~s~Téléporter le Joueur à vous", nil, {}, true, function(Hovered, Active, Selected, target)
        if (Selected) then
            ExecuteCommand("bring "..IdSelected)
            ESX.ShowAdvancedNotification("¦ULTIMATEV ", "", '~b~Vous venez de téléporter ~s~ '.. GetPlayerName(GetPlayerFromServerId(IdSelected)) ..' ~b~à vous~s~ !', "CHAR_STRETCH", 1)
        end
    end)
    ...
    RageUI.Button("→~s~ ~g~Heal le Joueur", "", {RightLabel = nilt}, true, function(Hovered, Active, Selected)
        if (Selected) then
            ...
            ESX.ShowAdvancedNotification("¦ULTIMATEV ", "", '~g~Heal de '.. GetPlayerName(GetPlayerFromServerId(IdSelected)) ..' ~g~effectué~s~ !', "CHAR_STRETCH", 1)
        end
    end)

    RageUI.Button("→~s~ ~b~Revive le Joueur",description, {RightLabel = nil}, true, function(Hovered, Active, Selected)
        if (Selected) then
            Frigo3 = Checked
            if Checked then
    ...
    RageUI.Separator("  ~y~Wipes et Gives ")

    if superadmin then
        RageUI.Button("→~s~Wipe l'inventaire du Joueur", "", {RightLabel = nil}, true, function(Hovered, Active, Selected)
            if (Selected) then
                ExecuteCommand("clearinventory "..IdSelected)
            ESX.ShowAdvancedNotification("¦ULTIMATEV ", "", "Vous venez de WIPE les items de ~b~".. GetPlayerName(GetPlayerFromServerId(IdSelected)) .."~s~ !", "CHAR_STRETCH", 1)
            end
        end)
    end
    if superadmin then
        RageUI.Button("→~s~Wipe les Armes du Joueur", "", {RightLabel = nil}, true, function(Hovered, Active, Selected)
            if (Selected) then
                ExecuteCommand("clearloadout "..IdSelected)
            ESX.ShowAdvancedNotification("¦ULTIMATEV ", "", "Vous venez de WIPE les armes de ~b~".. GetPlayerName(GetPlayerFromServerId(IdSelected)) .."~s~ !", "CHAR_STRETCH", 1)
            end
        end)
    end
    RageUI.Button("→~s~ ~b~Donner un Item au Joueur", "", {RightLabel = nil}, true, function(Hovered, Active, Selected)
        if (Selected) then
    ...
    if superadmin then
        RageUI.Button("→~s~Donner une Arme au Joueur ~r~(Remboursement seulement)", "", {RightLabel = nil}, true, function(Hovered, Active, Selected)
            if (Selected) then
...
RageUI.IsVisible(RMenu:Get('MenuAdmininspecteur', 'main7'), true, true, true, function()
    ...
    RageUI.Button("→ Gestion des Joueurs", "Gérer les joueurs connectés sur le serveur.", {RightLabel = ""},true, function()
    end, RMenu:Get('menu', 'gestion'))


    RageUI.Checkbox("→ Afficher les Noms (Pas en RP)", description, affichername,{},function(Hovered,Ative,Selected,Checked)
        if Selected then
    ...
    RageUI.Separator(CouleurRandom.."~s~- ~r~Actions Développement ~s~-")
                ...
                RageUI.Button("→ TP sur le Marqueur", "Se téléporter sur son marqueur.", {RightBadge = RageUI.BadgeStyle.tick}, true, function(Hovered, Active, Selected)
                    if (Selected) then
                        admin_tp_marker()
                    end
                            end)
...
---------------------------------------------------------------- Action Divers ----------------------------------------------------------------
                        ...
                        RageUI.IsVisible(RMenu:Get('MenuAdmininspecteur', 'main3'), true, true, true, function()
                            ...
                            RageUI.Separator("- Actions Personelles -")

                            RageUI.Button("→~s~ Vos Informations", "Consultez vos informations personnelles.", {RightLabel ="→→~s~"},true, function()
                            ...
                            RageUI.Checkbox("→~s~ Désactiver l'HUD ", description, Frigo,{},function(Hovered,Ative,Selected,Checked)
                                if Selected then
                                    Frigo = Checked
                                    if Checked then
                                        openCinematique()
                            ...
                            RageUI.Separator("- Actions de Gives -")

                            RageUI.Button("→~s~ Se donner de l'Armure",description, {RightBadge = RageUI.BadgeStyle.Armour}, true, function(Hovered, Active, Selected)
                                if (Selected) then
                            ...
                            RageUI.Button("→~s~ Se Soigner",description, {RightBadge = RageUI.BadgeStyle.Heart}, true, function(Hovered, Active, Selected)
                                if (Selected) then
                            ...
                            RageUI.Button("→~s~ Give d'Armes", "Se donner des armes.", {RightBadge = RageUI.BadgeStyle.Gun},true, function()
                            end, RMenu:Get('MenuAdmininspecteur', 'main5'))


                            RageUI.Button("→~s~ Changer d'Apparence", "Changer votre personnage.", {RightBadge = RageUI.BadgeStyle.Clothes},true, function()
...
---------------------------------------------------------------- Action Véhicule ----------------------------------------------------------------
                                        ...
                                        RageUI.IsVisible(RMenu:Get('MenuAdmininspecteur', 'main4'), true, true, true, function()
                                            ...
                                            RageUI.Separator("- Actions sur véhicules -")


                                            RageUI.Button("→~s~ Réparer le Véhicule", "Réparer le véhicule dans lequel vous êtes.", {RightBadge = RageUI.BadgeStyle.Heart}, true, function(Hovered, Active, Selected)
                                                if Selected then
                                               ...
                                               RageUI.Button("→~s~ Retourner le Véhicule", "Retourner le véhicule dans lequel vous êtes.", {RightBadge = RageUI.BadgeStyle.Heart}, true, function(Hovered, Active, Selected)
                                                if (Selected) then
                                                    admin_vehicle_flip()
                                                end
                                            end)
                                            RageUI.Button("→~s~ Spawn un Véhicule", "Faire apparaitre un véhicule.", {RightBadge = RageUI.BadgeStyle.Car}, true, function(_, _, Selected)
                                                if Selected then
                                        ...
                                        RageUI.Button("→~s~ Supprimer le Véhicule", "Supprimer le véhicule dans lequel vous êtes.", {RightBadge = RageUI.BadgeStyle.Car}, true, function(_, Active, Selected)
                                            if Active then
                                                ShowMarker()
                                            end
                                            if Selected then
                                        ...
                                        RageUI.Button("→~s~ Changer la Plaque", "Changer la plaque du véhicule dans lequel vous êtes.", {RightBadge = RageUI.BadgeStyle.Car}, true, function(_, Active, Selected)
                                            if Selected then
